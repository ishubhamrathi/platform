...
|---|---|---|
| `topic` | string | `eq` filter, e.g. `Java` (Supabase `topic=eq.Java` → `topic=Java`) |
...
```
Response `201`: created question. Toast on `409`/validation: see §6.

#### `PATCH /api/recall/questions/{id}` — Update own pending or ADMIN
Auth required. Only author or ADMIN (RLS equivalent). `400` if approved question edited by non-admin.

#### `DELETE /api/recall/questions/{id}`
Soft-delete (`deleted_at = now()`). Author or ADMIN.

#### Legacy fallback
`GET /api/content?type=question_sheets` and BDUI `QST` (`/api/v1/data/QST`) remain for admin console, but new feature work should use recall endpoints. Optional sync job copies approved `recall_questions` into `question_sheet` for backwards compat.
...
- Server derives `confidence_delta` from `action`; client may omit it (if sent, must match `+18/-12/0`).
- `questionId` must exist and be `approved` or own `pending`.
- Side effects: upsert `recall_streak_days` for today, bump `users.total_reviews`, recompute streak, increment session counters if `sessionId` provided.
...
Client mapping (unchanged from spec):
- Swipe Right → `know` (+18)
...
Auth required. Returns caller's review history for a question (most recent first). `200`: `{ "data": [ { "id","action","confidenceDelta","revealedAt","durationMs","createdAt" } ] }`
...
```json
{ "questionId": "uuid", "reviewCount": 4, "confidenceScore": 58, "level": "Familiar" }
```
When no `questionId`, returns array for all caller questions (used for Progress pie).

### 3.3 Bookmarks

#### `POST /api/recall/bookmarks` — body `{ "questionId": "uuid" }` → `201` (idempotent, `409` if already bookmarked maps to `200`)
...
```
Used by `Bookmarks.tsx`. Auth required. Joins `recall_bookmarks` → `recall_questions`.

### 3.4 Search

#### `GET /api/recall/search?q=hashmap&topic=Java&limit=20`
Debounced 200ms from `Search.tsx`. Equivalent to Supabase `rpc/search_questions`. Returns ranked questions (same shape as `GET /questions` paginated but without page wrapper for convenience — both forms supported).

Response `200`:
```json
{ "data": [ { "id":"...", "question":"...", "answer":"...", "tags":["hashmap"], "topic":"Java" } ], "total": 7 }
```
Query handling: `q` is `plainto_tsquery('english', q)`, ranking `ts_rank(to_tsvector(...), query)`, fallback `ILIKE` when `q` is empty.
...
```
`byTopic` computed from `recall_questions` left-joined to caller stats. `weekly` = last 7 days from `recall_streak_days`. `heatmap` = last 84 days. `streak` = consecutive days ending today.
...
Response `200`:
```json
{ "data": [{ "day":"2026-09-24","reviewsCount":5 }, ...], "streak": 12, "longestStreak": 28 }
```
...
```
Colors are a static map in backend (frontend may override).

### 3.7 Sessions & Streaks

#### `POST /api/recall/sessions` → `201` `{ "id":"uuid","startedAt":"..." }`
Auth required. Creates `recall_learning_sessions`. Call on Learn screen mount.

#### `PATCH /api/recall/sessions/{id}` — body `{ "endedAt":"...", "questionsSeen":12, "knows":8, "practices":4 }`
Auth required. Only owner. Response `200` `{ "id":"uuid","endedAt":"..." }`.
...
- `GET /api/auth/me` → `{ "id":"uuid","email":"a@b.com","name":"Shubham","role":"USER" }` + recall fields when `?include=recall` (streak, totalReviews).
- `PUT /api/recall/profile` (optional v1) → `{ "name","avatarUrl" }` — updates `users` + avatar via `/api/assets/upload`.
...
Frontend `AppContext` hydration:
```ts
const res = await fetch('/api/auth/me', { credentials: 'include' })
if (res.ok) setUser(await res.json())
```

All `/api/recall/*` mutations require session; they return `401` when missing and `403` when not owner/ADMIN.

Session config: `spring.session.store-type=jdbc`, `HttpSessionSecurityContextRepository`, `maximumSessions=1`, `withCredentials:true` on client (existing `frontend/src/api/client.ts` already sends `credentials:"include"`).

---

## 5. Frontend Integration Plan (no UI change outside data layer)

`src/store/AppContext.tsx` currently:
```ts
const [questions,setQuestions] = useState(mockQuestions)
```
Replace with (session-based, no Supabase SDK):
```ts
// list
...
const res = await fetch(`/api/recall/questions?q=${q}&size=20`, { credentials:'include' }).then(r=>r.json())
```
Field renames (server accepts both; response is camelCase to match original TS type):
- `interview_notes` → `interviewNotes`
- `follow_ups` → `followUps`
- `author_id` → `authorId`
- `is_community` → `isCommunity`
- `confidence_delta` → `confidenceDelta` (or omit, server derives)
- `question_id` → `questionId`

Auto-reveal stays client-side: `duration = clamp(1400 + question.length*22, 1800, 5200)`.

Add to `frontend/src/api/client.ts` (new `recall` namespace):
```ts
...
Contribution flow: `POST /api/recall/questions` with `isCommunity:true` → `status:pending` → ADMIN approves via `PATCH /api/recall/questions/{id}` `{ "status":"approved" }` or via BDUI `QST` widget.

---

## 6. Errors

Spring errors use the existing `ApiError` envelope (see `dto/ApiError.java`), not PostgREST format.

Success: `200/201` JSON, `204` delete. Errors:
...
Existing model reused: endpoints are `ADMIN` by default; dynamic PROJECT access requires a row in `api_access_rules` for a given `projectKey`. For recall:

- Public (no auth): `GET /api/recall/questions`, `GET /api/recall/questions/{id}`, `GET /api/recall/topics`, `GET /api/recall/search`
- Authenticated (`userPaths` in `SecurityConfig`): all `/api/recall/*` (session user). Service checks `currentUserId` for ownership.
- PROJECT (api-key): create client + key in Access Control UI, then add `PROJECT` rule with `pathPattern=/api/recall/**`. Key passed as `X-API-Key: <raw>` (via `ProjectApiKeyAuthFilter`). Content API `/api/content` stays unauthenticated as before.

Static public paths in `api-access.yaml` remain (health, auth, assets, content, ama ask, etc.). Recall paths are intentionally **not** in `api-access.yaml` so they stay ADMIN-by-default and are opened per-client via DB rules.

---

## 8. Versioning & Realtime

- `Prefer: api-version=1` header not required; breaking changes bump to `/api/recall/v2/*`.
- Realtime: v1 has no WebSocket. Frontend polls `GET /api/recall/questions?status=approved&sort=created_at.desc&size=5` after contribute or use `If-Modified-Since`. Future: `GET /api/recall/questions/stream` (SSE) + `supabase.channel` shim on frontend.
- Storage: `question-assets` bucket maps to existing Supabase bucket `platform_public_assets` via `POST /api/assets/upload`; short link `/a/{assetId}` unchanged.
...
# PROJECT api-key alternative (instead of cookie)
...
8. Verify CORS `AllowCredentials` + `VITE_API_BASE_URL` (already `credentials:include` in `frontend/src/api/client.ts:request`).
```
